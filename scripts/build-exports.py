#!/usr/bin/env python3
"""Build exports/catalog.json from validated atoms and compositions.

Walks atoms/ and documents/; validates each against its schema; assembles
a single machine-readable catalog manifest. Exits 1 on validation failure.

doc-atoms has no rules/ namespace — documentation is explanatory, not
normative, so it carries no compatibility rules between atoms.
"""
import json
import sys
...
COMPOSITIONS_DIR = REPO / "documents"
EXPORT_PATH = REPO / "exports" / "catalog.json"
CATALOG_NAME = "doc-atoms"
...
def main() -> int:
    ...
    print(f"wrote {EXPORT_PATH.relative_to(REPO)} — {len(atoms)} atoms, {len(compositions)} compositions")
    return 0
