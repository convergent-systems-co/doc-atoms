#!/usr/bin/env python3
"""Validate every atom and composition against its schema.

Per-file checks:
  atoms/<type>/<id>.json       → atom-v1.json; id == filename stem; type == parent dir
  documents/<id>.json          → composition-v1.json; id == filename stem
    ...
    for path in sorted(ATOMS_DIR.rglob("*.json")):
        ...
        if not local_errors:
            ...
            if data.get("type") != parent:
                ...
                key = f"doc-atoms://atoms/{parent}/{data['id']}"
                atom_index[key] = data
                print(f"✓ {rel}")
        errors += local_errors
...
def main() -> int:
    ...
    total = atom_errors + composition_errors
    if total:
        print(f"\n{total} error(s)")
        return 1
    print(f"\nall valid")
